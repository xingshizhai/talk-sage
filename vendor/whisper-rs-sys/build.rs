...
fn main() {
    ...
    if cfg!(feature = "vulkan") {
        ...
        // TalkSage patch: sherpa-onnx 官方静态库以 /MT（静态 CRT）编译，
        // whisper.cpp 默认 /MD 会导致 MSVC 运行时双重链接（LNK2005/LNK1169）。
        // 统一为静态 CRT，避免与 sherpa-onnx 冲突。
        //
        // 三层保险确保 /MT 生效：
        // 1. CMP0091=NEW：让 CMAKE_MSVC_RUNTIME_LIBRARY 成为唯一的 CRT 控制方式，
        //    阻止 CMake 把 /MD 写入 CMAKE_C_FLAGS_* 初始值；
        // 2. CMAKE_MSVC_RUNTIME_LIBRARY=MultiThreaded：显式指定静态 CRT；
        // 3. cflag/cxxflag("/MT")：覆盖 cmake crate 自动追加进 CMAKE_C_FLAGS 的 -MD
        //    （MSVC 命令行以最后出现的 /MD|/MT 为准，/MT 在 /MD 之后则 /MT 胜出）。
        //
        // 若旧缓存中 CMAKE_C_FLAGS* 已含 -MD，需删除 CMakeCache.txt 强制重新配置；
        // 下方代码检测并自动清除过期缓存，避免手动 cargo clean。
        if cfg!(target_os = "windows") {
            // 检测并清除含 -MD 的过期 CMake 缓存（仅首次修复时触发，之后缓存已正确）
            let cmake_cache = out.join("build").join("CMakeCache.txt");
            if cmake_cache.exists() {
                if let Ok(content) = std::fs::read_to_string(&cmake_cache) {
                    if content.contains("-MD") || content.contains("/MD") {
                        let _ = std::fs::remove_file(&cmake_cache);
                    }
                }
            }
            // CMP0091=NEW: 禁止 CMake 把 /MD 写入 CMAKE_C_FLAGS_* 初始值
            config.define("CMAKE_POLICY_DEFAULT_CMP0091", "NEW");
            config.define("CMAKE_MSVC_RUNTIME_LIBRARY", "MultiThreaded");
            // belt-and-suspenders: 在 cmake crate 追加的 -MD 之后再追加 /MT
            config.cflag("/MT");
            config.cxxflag("/MT");
        }
