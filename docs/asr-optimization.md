...
**最后更新：** 2026-08-28（场景化低延迟切段 + 切点尾音修复）
...
whisper 是段级引擎、没有 partial，一句话要等 VAD 自然停顿或场景的
`asr_segment_ms` 到期，然后再完成整段推理。

已实施的策略：

| 场景 | `asr_segment_ms` | 考量 |
|---|---:|---|
| 实时翻译 | 3000 | 优先出字速度 |
| 一对一 / 双语对话 / 自定义默认 | 4000 | 延迟与上下文平衡 |
| 单人听写 | 5000 | 保留较完整句子 |
| 多人会议 / 演讲课堂 | 6000 | 优先上下文和专业术语 |

内置场景只读，自定义场景可在设置页调整；`0` 表示只按 VAD 自然停顿切段。
流式引擎忽略该配置。

同时修正了强制切点的丢音频问题：达到阈值的当前 100ms 音频块现在会先送入
ASR 和声纹缓冲，再请求 `finish()`。旧顺序会直接丢掉该块，导致切点附近尾字消失。

仍可继续评估“快速 partial 预览 + Whisper 最终校正”双通路，但不将低准确率的
Paraformer 恢复为默认产品引擎。
